...
#include "associate_mesh.h"
    ...
    !       Compute squared vertical difference of velocity (Vr-Vd)**2 
    !        
    ...
    do node=1, myDim_nod2D !+eDim_nod2D
        ...
        ! Surface layer is our reference dVsq(m2/s2) --> dVsq(1,node)=0
        dVsq (nzmin,node) = 0.0_WP 
        
        ! dbsfc (buoyancy difference with respect to the surface (m/s2)) 
        ! is now computed in oce_ale_pressure_bv.F90
        ! Surface layer is our reference dbsfc(m/s2) --> dbsfc(1,node)=0
    ...
    do node=1, myDim_nod2D
        ...
        do nz=nzmin+1,nzmax-1
            ...
            if (nz < kbl(node)) then 
                ...
                ! outside the boundary layer set nonlocal terms to zero
                ghats(nz,node)=0.0_WP    
            endif 
        end do
    end do    
    ...
    !___________________________________________________________________________
    ! compute tracer viscosituers over elements
    ...
    !___________________________________________________________________________
    ! non-local contribution will be added to tracer RHS directly in 
...
#include "associate_mesh.h"
    ...
    !___________________________________________________________________________
    ! compute ocean boudnary layer depth and index 
    do node=1, myDim_nod2D !+eDim_nod2D
        nzmin = ulevels_nod2D(node)
        nzmax = nlevels_nod2D(node)
        
        ! set up short wave pentration coefficient
        if (use_sw_pene) then 
        ...
        do nz=nzmin+1,nzmax
            ...
            !___________________________________________________________________
            ! is surface buoyancy forcing stable :1 (bfsfc(+)) or instable:0 
            ! (bfsfc(-))
            ...
            !___________________________________________________________________
            ! compute the turbulent shear contribution to Rib --> eqn. (23)
            ! --> bvfreq computed in subroutine pressure_bv(mesh)
            ! --> bvfreq squared and bvsq are calculated @ zbar
            bvsq = bvfreq(nz ,node)                                   
                
            Vtsq = zk * ws * SQRT(ABS(bvsq)) * Vtc
            
            !___________________________________________________________________
            !  compute bulk Richardson number at new level --> eqn. (21)
            Ritop = zk    *   dbsfc( nz ,node )                
            Rib_k = Ritop /  (dVsq ( nz ,node ) + Vtsq + epsln )  ! dbsfc (m/s2) and dVsq @ Z are calculated under oce_mixing_kpp
            dzup  = zk    -   zkm1
            
            ! if layer nz bulk richardson number Rib_k exceed critical bulk 
            ! richardson number Ricr (=0.3)--> interpolate boundary layer depth
            if (Rib_k > Ricr) then
        ...
        end do ! --> do nz=nzmin+1,nzmax
       
        !_______________________________________________________________________
        ! find stability and buoyancy forcing at boundary layer depth hbl
        if (use_sw_pene) then
            ! Linear interpolation of sw_3d to depth of hbl
            ! --> nz commes here from the previous loop at point of exit
            bfsfc(node) = Bo(node) + & 
                            coeff_sw * (sw_3d(nzmin,node) &
                                        -(sw_3d(nz-1,node) + (sw_3d(nz,node)-sw_3d(nz-1,node)) &
                                        *(hbl(node)-zkm1 )/dzup) &
                                        )
            stable(node) = 0.5_WP + SIGN( 0.5_WP, bfsfc(node) )
    ...
    !___________________________________________________________________________
    ! smoooth ocean boundary layer depth (default: smooth_hbl=.false. )
    if (smooth_hbl) then
    ...
    do node=1, myDim_nod2D !+eDim_nod2D
        ...
        !_______________________________________________________________________
        ! find new kbl from alternated hbl
        kbl(node) = nzmax
        do nz=nzmin+1,nzmax
        ...
        if (use_sw_pene) then
            ! Linear interpolation of sw_3d to depth of actual/alternated hbl
            bfsfc(node) = Bo(node) + &
                            coeff_sw * (sw_3d(nzmin,node) &
...
#include "associate_mesh.h"
    ...
    !___________________________________________________________________________
    ! horizontal smoothing of richardson number
