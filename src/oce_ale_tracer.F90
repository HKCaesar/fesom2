...
#include "associate_mesh.h"
    ...
    ! -->   solve for T_new                                      /I\. nvec_up (+1)
    !                                                             I                 
    !    ----------- zbar_1, V_1 (Volume eq. to Area)        *----I----*
    ! Z_1 o T_1                                              |\   I  ./|
    !    ----------- zbar_2, V_2                             | \   ./  |   Gaus Theorem:
    ! Z_2 o T_2                                              |  \ /    |    --> Flux form
    !    ----------- zbar_3, V_3                             |   |     |    --> normal vec outwards facing
    ! Z_3 o T_3                                              *---|-----*
    !    ----------- zbar_4                                   \  | I ./
    !        :                                                 \ | I/ 
    !                                                           \|/I   
    ...
    ! V_i*Tnew_i *h_i = -dt * [ K_33 * (Tnew_i-1 - Tnew_i)/(Z_i-1 - Z_i) * V_i * nvec_up 
    !                          +K_33 * (Tnew_i - Tnew_i+1)/(Z_i - Z_i+1) * V_i+1 * nvec_dwn ]
    !     Tnew_i *h_i = -dt * [ K_33 * (Tnew_i-1 - Tnew_i)/(Z_i-1 - Z_i) * nvec_up 
    !                          +K_33 * (Tnew_i - Tnew_i+1)/(Z_i - Z_i+1) * V_i+1/V_i * nvec_dwn ]
    !                                                  |                                 |
    !                                                  v                                 v
    !                                         diffusive flux towards             diffusive flux towards
    !                                        T_2 trough upper boundary          T_2 trough lower boundary
    !                                                   V_i                              V_i+1
    ! --> 3rd. solve for coefficents a, b, c:
    ! f(Tnew) = [ a*dTnew_i-1 + b*dTnew_i + c*dTnew_i+1 ] 
    !
    !     df(Tnew)/dTnew_i-1 = a = -dt*K_33/(Z_i-1 - Z_i)
    ! 
    !     df(Tnew)/dTnew_i+1 = c =  dt * K_33 * 1/(Z_i - Z_i+1) * V_i+1/V_i * (nvec_dwn=-1)
    !                            = -dt * K_33 * 1/(Z_i - Z_i+1) * V_i+1/V_i
    !
    !     df(Tnew)/dTnew_i   = b = h_i + dt*K_33/(Z_i-1 - Z_i) * (nvec_up=+1)
    !                                  - dt*K_33/(Z_i - Z_i+1) * V_i+1/V_i * (nvec_dwn=-1)
    !                            = h_i + dt*K_33/(Z_i-1 - Z_i)
    !                                  + dt*K_33/(Z_i - Z_i+1) * V_i+1/V_i
    !                            = h_i -(a+c)
    !
    ...
    ! RHS/V_i =  K_33 * dt * (Tstar_i-1 - Tstar_i)/(Z_i-1 - Z_i) * (nvec_up=1)
    !          + K_33 * dt * (Tstar_i - Tstar_i+1)/(Z_i - Z_i+1) * V_i+1/V_i * (nvec_dwn=-1)
    !
    !         =  K_33*dt/(Z_i-1 - Z_i)             * Tstar_i-1
    !          - K_33*dt/(Z_i-1 - Z_i)             * Tstar_i
    !          - K_33*dt/(Z_i - Z_i+1) * V_i+1/V_i * Tstar_i
    !          + K_33*dt/(Z_i - Z_i+1) * V_i+1/V_i * Tstar_i+1
    !         
    ...
    do n=1,myDim_nod2D  
        ...
        !!PS zinv=1.0_WP*dt    ! no .../(zbar(1)-zbar(2)) because of  ALE
        ...
        a(nz)=0.0_WP
        c(nz)=-(Kv(nz+1,n)+Ty1)*zinv2*dt*area(nz+1,n)/area(nz,n)
        ...
        if (do_wimpl) then
            v_adv=dt*area(nz+1,n)/area(nz,n)
            b(nz)=b(nz)+Wvel_i(nz, n)*dt-min(0._WP, Wvel_i(nz+1, n))*v_adv
        ...
        !!PS zinv=1.0_WP*dt   ! no ... /(zbar(nzmax-1)-zbar(nzmax)) because of ale
        ...
        if (use_kpp_nonlclflx) then
            ...
            if     (mix_scheme_nmb==1 .or. mix_scheme_nmb==17) then
                if     (tr_num==1) then ! temperature
                    ...
                    nz = nzmin
                    tr(nz)=tr(nz) &
                               +(-MIN(ghats(nz+1,n)*blmc(nz+1,n,2), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * heat_flux(n) / vcpw * dt
                    !___bulk____________________________________________________
                    do nz=nzmin+1, nzmax-2
                        tr(nz)=tr(nz) &
                               +( MIN(ghats(nz  ,n)*blmc(nz  ,n,2), 1.0_WP)&
                                 -MIN(ghats(nz+1,n)*blmc(nz+1,n,2), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * heat_flux(n) / vcpw * dt
                    end do
                ...
                elseif (tr_num==2) then ! salinity
                    ...
                    nz = nzmin
                    tr(nz)=tr(nz) &
                               -(-MIN(ghats(nz+1,n)*blmc(nz+1,n,3), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * rsss * water_flux(n) * dt
                    !___bulk____________________________________________________            
                    do nz=nzmin+1, nzmax-2
                        tr(nz)=tr(nz) &
                               -( MIN(ghats(nz  ,n)*blmc(nz  ,n,3), 1.0_WP) &
                                 -MIN(ghats(nz+1,n)*blmc(nz+1,n,3), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * rsss * water_flux(n) * dt
                    end do
                    !___bottom__________________________________________________
            ...
            elseif (mix_scheme_nmb==3 .or. mix_scheme_nmb==37) then
                if     (tr_num==1) then ! temperature
                    ...
                    nz = nzmin
                    tr(nz)=tr(nz) &
                               +(-MIN(kpp_nonlcltranspT(nz+1,n)*kpp_oblmixc(nz+1,n,2), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * heat_flux(n) / vcpw * dt
                    !___bulk____________________________________________________
                    do nz=nzmin+1, nzmax-2
                        tr(nz)=tr(nz) &
                               +( MIN(kpp_nonlcltranspT(nz  ,n)*kpp_oblmixc(nz  ,n,2), 1.0_WP)&
                                 -MIN(kpp_nonlcltranspT(nz+1,n)*kpp_oblmixc(nz+1,n,2), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * heat_flux(n) / vcpw * dt
                    end do
                    !___bottom__________________________________________________
                ...
                elseif (tr_num==2) then ! salinity
                    ...
                    nz = nzmin
                    tr(nz)=tr(nz) &
                               -(-MIN(kpp_nonlcltranspS(nz+1,n)*kpp_oblmixc(nz+1,n,3), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * rsss * water_flux(n) * dt
                    !___bulk____________________________________________________            
                    do nz=nzmin+1, nzmax-2
                        tr(nz)=tr(nz) &
                               -( MIN(kpp_nonlcltranspS(nz  ,n)*kpp_oblmixc(nz  ,n,3), 1.0_WP) &
                                 -MIN(kpp_nonlcltranspS(nz+1,n)*kpp_oblmixc(nz+1,n,3), 1.0_WP)*area(nz+1,n)/area(nz,n) &
                                ) * rsss * water_flux(n) * dt
                    end do
                    !___bottom__________________________________________________
